...
% for i=1:x
%     fprintf('Enter joint parameter %d ',i);
%     jtconfig(i)=input(' ');
% end
jsingu(J);

fprintf('Do you want to find if a particular pose is close to singularity?\nPress y for yes and n for no \n');
pose=input('y or n: ','s');
if xor(strcmp(pose,'y'),strcmp(pose,'n')) == 0
    fprintf('Invalid input. Press y for yes, Press n for no\n');
    %exit;
end
if pose~='n'
    for i=1:x
    ...
    singularity= R.maniplty(jtconfig)
    fprintf('If this values is close to 0 (generally if the order is higher than 10e-3) then\n the given pose is close to singularity'); 
end
